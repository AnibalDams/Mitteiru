  ...
  <ModalWithoutActions bind:showModal>
    ...
    <p style="display: inline-block; margin-top:10px;margin-bottom:10px; ">
     ...
     <div style="display:flex;gap:5px">
      ...
      {data.anime.name} episode {episodeNumber}</h2>
      <button
        title="Synopsis"
        on:click={() => {
          showModal = true;
      ...
      {#if episode_.external}<span
          style="display: inline-block;font-size:16px; padding:10px;background-color:rgb(255, 79, 79);color:darkred; border-radius:10px; margin:10px;"
...
<style>
  ...
  .episode_container .top_information {
    display: flex;
