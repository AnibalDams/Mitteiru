...
>
  ...
  <img
    ...
    src={dataA.similarAnime.studio === "animeFlv"
      ? dataA.similarAnime.cover
      : dataA.similarAnime.horizontalImage}
    alt=""
  />
  <span class="related_anime_card_name"
    >{dataA.similarAnime.name.length > 45
      ? `${dataA.similarAnime.name.substring(0, 45)}...`
      : dataA.similarAnime.name}{#if inList}
      <SavedCheck />
    {/if}</span
  >
</div>
