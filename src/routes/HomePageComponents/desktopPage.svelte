...
{#if loaded}
  ...
  <AnimeReccomendation animeData={animeRandom} {animeGenres} inList={animesInList.find(e=>e.id==animeRandom.id)?true:false} />
  ...
  <div class="anime_card_container">
    {#each mostPopularAnimes as popularAnime}
      <AnimeCard animeData={popularAnime} saved={animesInList.find(e=>e.id==popularAnime.id)?true:false}/>
    {/each}
  </div>
{:else}
  <div
