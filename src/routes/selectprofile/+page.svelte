<script>
    ...
    const refreshProfiles = async ()=>{
        ...
        if (userId.length > 0 && userId !=undefined) {
            const profilesQuery = await axios(`http://localhost:8000/user/${userId}/profile/all`)
...
{#if logged}
<Header logged={logged} profileImage={profileImage} name={pName}/>
...
<div class="container">
    ...
    <span class="title">Who's watching?   <button on:click={refreshProfiles} class="reload_button"><Reload/></button></span>
  
    <div class="profile_container">

        {#each profiles as profile}
            <Profile secondaryAction={refreshProfiles} onClick={()=>{
                setProfile(profile[0],profile[2],profile[3])
            }} image={profile[2]} id={profile[0]} name={profile[3]}/>
    ...
    {:else}
        ...
        <NewProfilePage userId={userId} refresh={refreshProfiles} cancel={()=>{newProfile=false}}/>
    {/if}
